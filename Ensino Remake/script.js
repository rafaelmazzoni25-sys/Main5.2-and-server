...
const bmdDownloadWindowsButton = document.getElementById("bmd-download-windows");
const bmdDownloadUnixButton = document.getElementById("bmd-download-unix");
const bmdScriptStatus = document.getElementById("bmd-script-status");
...
const BMD_WINDOWS_FILENAME = "converter_bmd.bat";
const BMD_UNIX_FILENAME = "converter_bmd.sh";
...
function escapeWindowsEnv(value) {
...
function buildBmdFlagList(config) {
    const flags = [`--frame-time=${config.frameTime.toFixed(6)}`];
    if (config.findTextures) {
        flags.push("--find-textures");
    }
    if (config.renameTextures) {
        flags.push("--rename-textures");
    }
    if (config.exportNormals) {
        flags.push("--export-normals");
    }
    return flags;
}

function buildBmdWindowsScript(config) {
    if (bmdDetectedFiles.length === 0) {
        return "";
    }

    const rootRaw = config.rootPath ? toWindowsPath(config.rootPath.trim()) : "%~dp0..\\Data";
    const outRaw = config.outputDir ? toWindowsPath(config.outputDir.trim()) : "%~dp0output";
    const rootValue = config.rootPath ? escapeWindowsEnv(rootRaw) : rootRaw;
    const outValue = config.outputDir ? escapeWindowsEnv(outRaw) : outRaw;
    const files = bmdDetectedFiles.map((file) => escapeWindowsEnv(toWindowsPath(file)));
    const flags = buildBmdFlagList(config);

    const lines = [
        "@echo off",
        "setlocal enabledelayedexpansion",
        "REM Ajuste o caminho abaixo se o executável estiver em outra pasta.",
        "set \"TOOL=%~dp0BMD_FBX_CLI.exe\"",
        `set "ROOT=${rootValue}"`,
        `set "OUT=${outValue}"`,
        `set "FLAGS=${flags.join(" ")}"`,
        "",
        "if not exist \"%TOOL%\" (",
        "    echo [ERRO] Nao foi possivel localizar %TOOL%",
        "    exit /b 1",
        ")",
        "",
        "if not exist \"%OUT%\" mkdir \"%OUT%\"",
        "",
        "echo === Iniciando conversao com BMD_FBX ===",
        "for %%F in ("
    ];
    ...
    lines.push(
        ") do (",
        "    echo [BMD_FBX] Convertendo %%~F",
        "    \"%TOOL%\" %FLAGS% --root=\"%ROOT%\" --input \"%ROOT%\\%%~F\" --output \"%OUT%\"",
        "    if errorlevel 1 (",
...
function buildBmdUnixScript(config) {
    if (bmdDetectedFiles.length === 0) {
        return "";
    }

    const defaultRoot = "${SCRIPT_DIR}/../Data";
    const defaultOut = "${SCRIPT_DIR}/output";
    const rootRaw = config.rootPath ? toPosixPath(config.rootPath.trim()) : defaultRoot;
    const outRaw = config.outputDir ? toPosixPath(config.outputDir.trim()) : defaultOut;
    const files = bmdDetectedFiles.map((file) => escapeShellPath(file));
    const flags = buildBmdFlagList(config)
        .map((flag) => `"${flag}"`)
        .join(" ");
    ...
    const lines = [
        ...
        `ROOT="${escapeShellPath(rootRaw)}"`,
        `OUT="${escapeShellPath(outRaw)}"`,
        'TOOL="${TOOL:-${SCRIPT_DIR}/BMD_FBX_CLI}"',
        `FLAGS=(${flags})`,
        "",
        'if [[ ! -x "$TOOL" ]]; then',
        '    echo "[ERRO] Nao foi possivel localizar $TOOL" >&2',
        "    exit 1",
        "fi",
        "",
        'mkdir -p "$OUT"',
    ...
    lines.push(
        ...
        '    echo "[BMD_FBX] Convertendo $file"',
        '    if ! "$TOOL" "${FLAGS[@]}" --root "$ROOT" --input "$ROOT/$file" --output "$OUT"; then',
        '        echo "[ERRO] Falha ao converter $file" >&2',
        "    fi",
        "done",
        "",
        'echo "=== Processo concluido ==="',
...
function buildBmdScriptFile(config, platform) {
    if (platform === "windows") {
        return { filename: BMD_WINDOWS_FILENAME, content: buildBmdWindowsScript(config) };
    }
    if (platform === "unix") {
        return { filename: BMD_UNIX_FILENAME, content: buildBmdUnixScript(config) };
    }
...
function setBmdScriptStatus(message, tone = "muted") {
    if (!bmdScriptStatus) {
        return;
    }

    const toneClasses = ["text-muted", "text-success", "text-warning", "text-danger", "text-info"];
    bmdScriptStatus.classList.remove(...toneClasses);
    ...
    bmdScriptStatus.classList.add(toneClass);
    bmdScriptStatus.textContent = message;
}

function handleBmdDownload(platform) {
    if (bmdDetectedFiles.length === 0) {
        setBmdScriptStatus("Nenhum arquivo .BMD/.SMD selecionado para conversão.", "warning");
        return;
    }

    const config = getBmdConfig();
    const { filename, content } = buildBmdScriptFile(config, platform);

    if (!content.trim()) {
        setBmdScriptStatus("Não foi possível gerar o script solicitado. Verifique as opções preenchidas.", "danger");
        return;
    }

    const mime = platform === "windows" ? "application/x-bat" : "text/x-shellscript";
    triggerFileDownload(filename, content, mime);

    const label = platform === "windows" ? "Windows (.bat)" : "Unix/macOS (.sh)";
    setBmdScriptStatus(
        `Script ${label} gerado com as opções atuais. Execute-o na mesma pasta do BMD_FBX_CLI compilado para iniciar a conversão.`,
        "success"
    );
}
...
function buildBmdSnippet(config) {
    ...
    if (snippetFiles.length > 0) {
        const sampleLines = snippetFiles.map((file) => `//   ${file}`);
        if (extraCount > 0) {
            sampleLines.push(`//   ... +${numberFormatter.format(extraCount)} arquivo(s) adicional(is)`);
        }
        sampleBlock = `

// Exemplos detectados na varredura:
${sampleLines.join("
")}`;
    }

    return `#include "BMD_FBX.h"
#include <filesystem>
#include <iostream>
#include <optional>
#include <string>
...
struct CliOptions {
    ...
    std::string rootDir;
    double frameTime = ${config.frameTime.toFixed(6)};
    bool findTextures = ${config.findTextures ? "true" : "false"};
    bool renameTextures = ${config.renameTextures ? "true" : "false"};
    bool exportNormals = ${config.exportNormals ? "true" : "false"};
};

void PrintUsage(const char* exeName)
{
    const char* tool = exeName ? exeName : "BMD_FBX_CLI";
    std::cerr << "Uso: " << tool
              << " --input caminho.bmd [--output pasta] [--root pastaData]"
              << " [--frame-time segundos] [--find-textures] [--rename-textures] [--export-normals]" << '\n';
    std::cerr << "Exemplo: " << tool
              << " --root C:/Main/Data --output ./fbx --frame-time ${config.frameTime.toFixed(6)} --find-textures --rename-textures --export-normals --input Character/Exemplo.bmd" << '\n';
}

bool ParseArgs(int argc, char** argv, CliOptions& options, std::string& error)
{
    auto requireValue = [&](int& index, const char* flag) -> std::optional<std::string> {
        if (index + 1 >= argc) {
            error = std::string("Parâmetro exige valor: ") + flag;
            return std::nullopt;
        }
        return std::string(argv[++index]);
    };

    auto applyFrameTime = [&](const std::string& value) -> bool {
        try {
            double parsed = std::stod(value);
            if (parsed <= 0.0) {
                return false;
            }
            options.frameTime = parsed;
            return true;
        } catch (...) {
            return false;
        }
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--input") {
            auto value = requireValue(i, "--input");
            if (!value) {
                return false;
            }
            ...
            auto value = requireValue(i, "--output");
            if (!value) {
                return false;
            }
            options.outputDir = std::move(*value);
        } else if (arg.rfind("--output=", 0) == 0) {
            options.outputDir = arg.substr(9);
        } else if (arg == "--root") {
            auto value = requireValue(i, "--root");
            if (!value) {
                return false;
            }
            options.rootDir = std::move(*value);
        } else if (arg.rfind("--root=", 0) == 0) {
            options.rootDir = arg.substr(7);
        } else if (arg == "--frame-time") {
            auto value = requireValue(i, "--frame-time");
            if (!value || !applyFrameTime(*value)) {
                error = "Valor inválido para --frame-time";
                return false;
            }
        } else if (arg.rfind("--frame-time=", 0) == 0) {
            if (!applyFrameTime(arg.substr(13))) {
                error = "Valor inválido para --frame-time";
                return false;
            }
        } else if (arg == "--find-textures") {
            options.findTextures = true;
        } else if (arg == "--rename-textures") {
            options.renameTextures = true;
        } else if (arg == "--export-normals") {
            options.exportNormals = true;
        } else if (arg == "--help" || arg == "-h") {
            error.clear();
            return false;
        } else {
            error = std::string("Parâmetro desconhecido: ") + arg;
            return false;
        }
    }

    if (options.input.empty()) {
        error = "Informe o caminho do arquivo com --input <arquivo.bmd>";
        return false;
    }
...
int main(int argc, char** argv)
{
    ...
    if (!ParseArgs(argc, argv, options, error)) {
        if (!error.empty()) {
            std::cerr << "[BMD_FBX] " << error << '\n';
        }
        PrintUsage(argv[0]);
        return error.empty() ? 0 : 1;
    }

    fs::path inputPath = fs::u8path(options.input);
    if (!fs::exists(inputPath)) {
        std::cerr << "[BMD_FBX] Arquivo não encontrado: " << inputPath.u8string() << '\n';
        return 1;
    }

    if (!options.rootDir.empty()) {
        BMD_FBX::SetRootPath(fs::u8path(options.rootDir));
    } else {
        BMD_FBX::SetRootPath(inputPath.parent_path());
    }
    BMD_FBX::SetFrameTime(options.frameTime);

    BMD_FBX converter;

    std::string inputUtf8 = inputPath.u8string();
    std::string outputBuffer;
    const char* outputPtr = nullptr;
    if (!options.outputDir.empty()) {
        fs::path outDir = fs::u8path(options.outputDir);
        std::error_code dirError;
        fs::create_directories(outDir, dirError);
        if (dirError) {
            std::cerr << "[BMD_FBX] Aviso: " << dirError.message() << " ao criar " << outDir.u8string() << '\n';
        }
        std::string fileStem = inputPath.stem().u8string();
        fs::path destPath = outDir / (fileStem + ".fbx");
        outputBuffer = destPath.u8string();
        outputPtr = outputBuffer.c_str();
    }

    if (!converter.Unpack(inputUtf8.c_str(), outputPtr, options.findTextures, options.renameTextures, options.exportNormals)) {
        std::cerr << "[BMD_FBX] Falha ao converter " << inputUtf8 << '\n';
        return 2;
    }

    std::cout << "[BMD_FBX] Conversão concluída: " << inputUtf8 << '\n';
    return 0;
}${sampleBlock}`;
}
...
function refreshBmdSnippet() {
    ...
    if (bmdOptionsSummary) {
        ...
        const rootInfo = config.rootPath
            ? `Raiz: ${config.rootPath}`
            : "Raiz padrão: script/../Data";
        const outInfo = config.outputDir
            ? `Saída: ${config.outputDir}`
            : "Saída padrão: script/output";
        bmdOptionsSummary.textContent = `Opções atuais — ${parts.join(" · ")}. ${rootInfo}. ${outInfo}. Snippet inclui ${numberFormatter.format(
            snippetCount
...
function updateBmdConfigurator(files = []) {
    ...
    if (!files || files.length === 0) {
        ...
        refreshBmdSnippet();
        bmdDownloadWindowsButton && (bmdDownloadWindowsButton.disabled = true);
        bmdDownloadUnixButton && (bmdDownloadUnixButton.disabled = true);
        setBmdScriptStatus("Selecione a pasta Data para gerar scripts automáticos de conversão.", "muted");
        return;
    }
    ...
    if (bmdDownloadWindowsButton) {
        bmdDownloadWindowsButton.disabled = false;
    }
    if (bmdDownloadUnixButton) {
        bmdDownloadUnixButton.disabled = false;
    }

    setBmdScriptStatus(
        "Pronto para exportar scripts .bat ou .sh. Ajuste os caminhos acima para refletir sua estrutura local antes de baixar.",
        "info"
    );
}
...
if (bmdDownloadWindowsButton) {
    bmdDownloadWindowsButton.addEventListener("click", () => handleBmdDownload("windows"));
}

if (bmdDownloadUnixButton) {
    bmdDownloadUnixButton.addEventListener("click", () => handleBmdDownload("unix"));
}
